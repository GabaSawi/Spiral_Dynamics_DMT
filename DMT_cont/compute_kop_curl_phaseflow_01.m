%% Step 1 of 4: Computing KOP and Curl for Kuramoto and Spiral Vorticity and Turbulence
% INPUTS:
...
% OUTPUTS:
% Coordinates on a regular grid -> % xCord = -250:downSRate:250; yCord = -150:downSRate:200; --> [X_grid, Y_grid] = meshgrid(xCord, yCord);
% KuraVertexGrid', 'curlz', 'vPhaseX', 'vPhaseY' (time evolution for each regular grid element) 
...
addpath(genpath('/...')); % Raw data matrix
...
% LH data, variables: DMT_all_left and PCB_all_left, structures with
% {SUBJECT}, then each cell has arrays (vertices,time)
load("data_LH.mat"); % LH data
% load('data_RH.mat'); % RH data
